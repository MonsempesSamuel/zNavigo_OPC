...
{ 
    ...
    // G�n�ration des onglets et chargement de la page d'accueil
    ...
    // D�finition de quelques propri�t�s de la fen�tre
...
void FenPrincipale::creerActions()
{
    ...
    actionGo = new QAction(QIcon("images/go.png"), tr("A&ller �"), this);
...
{
    QMessageBox::information(this, tr("A propos..."), tr("zNavigo est un projet r�alis� pour illustrer les tutoriels C++ du <a href=\"http://www.siteduzero.com\">Site du Z�ro</a>.<br />Les images de ce programme ont �t� cr��es par <a href=\"http://www.everaldo.com\">Everaldo Coelho</a>"));
}
...
void FenPrincipale::chargerPage()
{
    ...
    // On rajoute le "http://" s'il n'est pas d�j� dans l'adresse
    if (url.left(7) != "http://")
    {
        url = "http://" + url;
...
void FenPrincipale::changementTitre(const QString & titreComplet)
{
    ...
    // On tronque le titre pour �viter des onglets trop larges
...
{
    progression->setVisible(false);
    statusBar()->showMessage(tr("Pr�t"), 2000);
}
...
QWidget *FenPrincipale::creerOngletPageWeb(QString url)
{
    ...
    // Gestion des signaux envoy�s par la page web
